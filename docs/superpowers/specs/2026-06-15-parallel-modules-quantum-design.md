...
- v1 编排基建只做**最小集**（顺序 `agent()` + 控制流关卡 + schema 返回），`parallel/pipeline/resume/budget` 推后（§5.6）。
...
### 5.6 v1 最小集与推后项（YAGNI，待 review 确认）

- **v1 做**：`agent()` 顺序调用 + `{schema}` 结构化返回 + JS 控制流关卡 + 脚本加载/存储 + 跨 subagent 审批路由。
- **推后**：`parallel()` / `pipeline()`（真并行，等出现"并行试多个选型"等需求再加）、resume/journal、token budget、worktree 隔离。
- **确定性约束**：脚本内禁 `Date.now()`/`Math.random()`/无参 `new Date()`（为日后 resume 留路；时间戳走 `args`）。
...
2. **阶段 0b（地基，agent core）**：实现 §5 编排基建最小集——`agent()` + schema 返回 + 控制流关卡 + 脚本加载/存储 + 跨 subagent 审批路由。
...
- **v1 编排基建最小集**（顺序 agent + 控制流 + schema），**v1 首个量子适配器 = isQ**（用户决定）。
