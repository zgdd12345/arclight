...
| **M1 slice 3** | Write-ownership transfer: `POST/PATCH/DELETE /api/projects` → Python (Python SQLite write layer + deterministic `ORDER BY` + one-writer-per-table), then `files`/`memories` | ▶ **next** | — | (to write) |
...
| **M3** | `sessions`/loop + SSE (the heart): Python async-generator queryLoop, borrow engine submodules, epoch DB-trigger guard. **Also migrates `/api/config` + grants** (in-memory runtime, split-brain until the loop moves) | ⏳ later | — | — |
...
- **M1 slice 3:** when `workspaces` writes move to Python, add a deterministic `ORDER BY` (both runtimes currently rely on implicit rowid order) and enforce one-writer-per-table (TS must stop writing `workspaces`).
