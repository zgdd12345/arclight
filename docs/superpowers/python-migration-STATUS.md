...
| **M1 slice 3** | Write-ownership transfer: **PATCH+DELETE** `/api/projects` → Python (first Python SQLite write layer, `foreign_keys ON` cascade, deterministic `ORDER BY rowid`, exact-match proxy routing). **POST deferred to M3.** | ✅ **done** (2026-06-18) | `4c3020f` | [m1-slice3](plans/2026-06-18-python-core-migration-m1-slice3.md) |
