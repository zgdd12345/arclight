...
| **M1 slice 5** | `/api/files` upload (`POST /:id/files`) → Python — multipart + disk write to `.arclight/uploads/` + filename sanitization; reads M3 `sessions` for the workspace path (read-only). Writes NO DB table | ▶ **next** | — | (to write) |
