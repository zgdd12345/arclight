...
describe("不变量 1+2：事件序快照 / seq 连续 / epoch 仅压缩边界（slice2 恒 0）", () => {
  test("纯文本 turn：message.delta+ → turn.completed（turn.started 由 runner 准入时发出）", async () => {
    ...
    expect(events.map((e) => e.t)).toEqual(["message.delta", "turn.completed"]);
    expect(events.map((e) => e.seq)).toEqual([1, 2]); // 连续无缺口
...
describe("不变量 5：中断后无悬挂（abort 双路径）", () => {
  ...
  test("消费者提前 return()（SSE 断连路径）→ generator 清理且不再产出", async () => {
    ...
    await gen.next(); // 首个产出（message.delta；turn.started 已上移 runner）
